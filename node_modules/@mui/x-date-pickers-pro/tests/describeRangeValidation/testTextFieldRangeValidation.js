...
export const testTextFieldRangeValidation = (ElementToTest, getOptions) => {
  const {
    componentFamily,
    render,
    isSingleInput,
  ...
  describe('text field:', () => {
    ...
    it('should apply shouldDisableDate specifically on end date', function test() {
      ...
      const {
        ...
        onError: onErrorMock,
        value: dateParser([[2018, 2, 9], [2018, 2, 10]]),
        shouldDisableDate: (date, position) => position === 'end' ? adapterToUse.isAfter(date, adapterToUse.date(new Date(2018, 2, 10))) : false
      }));
      expect(onErrorMock.callCount).to.equal(0);
      testInvalidStatus([false, false], isSingleInput);
      setProps({
      ...
      setProps({
        value: dateParser([[2018, 2, 12], [2018, 2, 13]]),
        shouldDisableDate: (date, position) => position === 'end' ? adapterToUse.isBefore(date, adapterToUse.date(new Date(2018, 2, 13))) : false
      });
      expect(onErrorMock.callCount).to.equal(2);
    ...
    it('should apply shouldDisableDate specifically on start date', function test() {
      ...
      const {
        ...
        onError: onErrorMock,
        value: dateParser([[2018, 2, 9], [2018, 2, 10]]),
        shouldDisableDate: (date, position) => position === 'start' ? adapterToUse.isAfter(date, adapterToUse.date(new Date(2018, 2, 10))) : false
      }));
      expect(onErrorMock.callCount).to.equal(0);
      testInvalidStatus([false, false], isSingleInput);
      setProps({
      ...
      testInvalidStatus([true, false], isSingleInput);
      setProps({
        shouldDisableDate: (date, position) => position === 'start' ? adapterToUse.isBefore(date, adapterToUse.date(new Date(2018, 2, 13))) : false
      });
      expect(onErrorMock.callCount).to.equal(1);
      testInvalidStatus([true, false], isSingleInput);
    });
